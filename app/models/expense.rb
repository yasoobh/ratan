class Expense < ApplicationRecord
  ...
  def process_raw_expense(senderAddress, messageContent, messageTime)
    senderAddress.downcase!
    messageContent.downcase!

    if filter_message_content(messageContent)
      messageContent
    end
  end
...
# /netbanking/
  def get_merchant_name_from_raw_expense(messageContent)
    merchant_1 = /via (.*?) on/
    merchant_2 = /at ([A-Za-z\+ ]*?) on/
  ...
  def get_money_from_raw_expense(messageContentDc)
    # for Rs.99.00 + debit
    # INR 99 + debit
  ...
  # Keep INR
  # Keep debited
  def filter_message_content(messageContent)
    ...
    discardedKeywords.each { |discardedKeyword|
      regexMatch = discardedKeyword.match(messageContent)
      if not regexMatch.nil?
        return false
      end
    }
    ...
    includedKeywords.each { |includedKeyword|
      regexMatch = includedKeyword.match(messageContent)
      if not regexMatch.nil?
        return true
      end
    }
